...
set timestamp [clock format [clock seconds] -format "%d-%m-%Y__%H-%M-%S"]

report_timing -path full -delay max -max_paths 10 \
    > $REPORTS_DIR/timing_setup_$timestamp.rpt

report_timing -path full -delay min -max_paths 10 \
    > $REPORTS_DIR/timing_hold_$timestamp.rpt

# Area report
report_area -hierarchy \
    > $REPORTS_DIR/area_$timestamp.rpt

# Power report
report_power -hierarchy \
    > $REPORTS_DIR/power_$timestamp.rpt

# Design rule checks
report_constraint -all_violators \
    > $REPORTS_DIR/constraints_$timestamp.rpt

report_cell \
    > $REPORTS_DIR/cells_$timestamp.rpt

report_net \
    > $REPORTS_DIR/nets_$timestamp.rpt

# QoR summary
report_qor \
    > $REPORTS_DIR/qor_$timestamp.rpt
